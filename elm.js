...
var author$project$Main$ChecklistRun = F2(
	function (checklist, currentStep) {
		return {checklist: checklist, currentStep: currentStep};
	});
var author$project$Main$Create = function (a) {
	return {$: 'Create', a: a};
};
var author$project$Main$NewChecklistParameters = F4(
	function (name, editingName, steps, editingStep) {
		return {editingName: editingName, editingStep: editingStep, name: name, steps: steps};
	});
...
var author$project$Main$updateCreate = F2(
	function (msg, model) {
		switch (msg.$) {
			case 'UpdateTitle':
				var newTitle = msg.a;
				return _Utils_update(
					model,
					{
						editingName: elm$core$Maybe$Just(newTitle)
					});
			case 'SaveTitle':
				var _n1 = model.editingName;
				if (_n1.$ === 'Nothing') {
					return model;
				} else {
					var editingName = _n1.a;
					return _Utils_update(
						model,
						{editingName: elm$core$Maybe$Nothing, name: editingName});
				}
			case 'UpdateStep':
				var name = msg.a;
				return _Utils_update(
					model,
					{
						editingStep: elm$core$Maybe$Just(
							author$project$Main$Step(name))
					});
			default:
				var _n2 = model.editingStep;
				if (_n2.$ === 'Nothing') {
					return model;
				} else {
					var editingStep = _n2.a;
					return _Utils_update(
						model,
						{
...
var author$project$Main$UpdateChecklist = function (a) {
	return {$: 'UpdateChecklist', a: a};
};
var author$project$Main$UpdateStep = function (a) {
...
var author$project$Main$viewChecklistParametersEmpty = function (titleValue) {
	return A2(
		elm$html$Html$div,
		_List_Nil,
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						elm$html$Html$text('What\'s the name of your checklist?')
					])),
				A2(
				elm$html$Html$input,
				_List_fromArray(
					[
						...
						elm$html$Html$Attributes$class('mt-2 px-1 py-2 italic border w-full'),
						elm$html$Html$Attributes$placeholder('My awesome checklist'),
						elm$html$Html$Attributes$autofocus(true),
						elm$html$Html$Attributes$value(titleValue),
						elm$html$Html$Events$onInput(
						function (i) {
							return author$project$Main$UpdateChecklist(
								author$project$Main$UpdateTitle(i));
						}),
...
var author$project$Main$viewChecklistParameters = function (parameters) {
	...
	if (_n0.$ === 'Just') {
		var name = _n0.a;
		return author$project$Main$viewChecklistParametersEmpty(name);
	} else {
		var value_ = function () {
			var _n1 = parameters.editingStep;
			if (_n1.$ === 'Nothing') {
				return '';
			} else {
				var step = _n1.a;
		...
		var placeholder_ = elm$core$List$isEmpty(parameters.steps) ? 'First step' : 'Next step';
		return A2(
			elm$html$Html$div,
			_List_Nil,
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							elm$html$Html$text(parameters.name)
						])),
					A2(
					elm$html$Html$div,
					_List_Nil,
						...
						[
							...
							_List_fromArray(
								[
									...
									function (i) {
										return author$project$Main$UpdateChecklist(
											author$project$Main$UpdateStep(i));
									}),
									author$project$Main$onEnter(
									author$project$Main$UpdateChecklist(author$project$Main$SaveStep))
								]),
							_List_Nil)
						])),
					A2(
					...
					_List_fromArray(
						[
							elm$html$Html$Attributes$class('italic text-sm w-full text-right text-gray-500')
						]),
					_List_fromArray(
						[
							elm$html$Html$text('Press Enter to add')
						])),
					A2(
					elm$html$Html$div,
					_List_fromArray(
						[
							elm$html$Html$Attributes$class('text-right w-full mt-2')
						]),
					_List_fromArray(
						[
							A2(
...
var author$project$Main$isCompleted = function (checklistRun) {
	return _Utils_cmp(
		checklistRun.currentStep,
		elm$core$List$length(checklistRun.checklist.steps)) > -1;
};
var author$project$Main$makeViewModel = F3(
	function (step, index, currentIndex) {
		return (_Utils_cmp(index, currentIndex) < 0) ? {active: false, completed: true, text: step.name} : (_Utils_eq(index, currentIndex) ? {active: true, completed: false, text: step.name} : {active: false, completed: false, text: step.name});
	});
var author$project$Main$makeViewDataForSteps = function (checklistRun) {
	return A2(
		elm$core$List$indexedMap,
		F2(
			function (index, value) {
				return A3(author$project$Main$makeViewModel, value, index, checklistRun.currentStep);
			}),
		checklistRun.checklist.steps);
};
var author$project$Main$MoveToNext = {$: 'MoveToNext'};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						elm$html$Html$text(viewData.text)
					]))
			]));
};
var author$project$Main$viewChecklistRun = function (checklistRun) {
	var stepsViewData = author$project$Main$makeViewDataForSteps(checklistRun);
	return A2(
		elm$html$Html$div,
		_List_Nil,
		_List_fromArray(
